...
class PreferentialAttachmentModel(BaseGraph):
    def __init__(self,m,k,t_0,t_max,community_affinities, initial_graph=None):
        super(PreferentialAttachmentModel, self).__init__(edges=None, adjacency_matrix=None)
        ...
        if self._adjacency_matrix is None:
            for t in range(self.t_0,self.t_max):
                ...
                for e in range(self.m):
                    ...
                    p = [(self.get_community_affinity(t,j))*(edge_ends.count(j) + edge_starts.count(j)) for j in self._vertices]
            ...
            self._adjacency_matrix = Converter.convert_from_edge_list_to_adjacency_matrix(self.t_max, self._edges)
        return self._adjacency_matrix


    def get_community_affinity(self,i,j):
        return self._community_affinities[self._vertex_communities[i],self._vertex_communities[j]]